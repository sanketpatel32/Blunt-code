...
export function ConfirmationDialog({ title, description, confirmLabel, busy, onCancel, onConfirm, tone = 'destructive' }: { title: string; description: string; confirmLabel: string; busy: boolean; onCancel: () => void; onConfirm: () => void; /** destructive (default) for irreversible actions; primary for safe starts like running a scan. */ tone?: 'destructive' | 'primary' }) {
  const cancelRef = useRef<HTMLButtonElement>(null);
  const { dialogRef, onBackdropMouseDown } = useDialogA11y({ onClose: onCancel, busy, autoFocusRef: cancelRef });
  return (
  // biome-ignore lint/a11y/noStaticElementInteractions: backdrop-only dismissal is pointer convenience; keyboard users close via Escape and the dialog's own Cancel/close button.
    <div className="dialog-backdrop" role="presentation" onMouseDown={onBackdropMouseDown}><dialog ref={dialogRef} open aria-modal="true" aria-labelledby="confirmation-title"><div className="confirmation-dialog"><header><h2 id="confirmation-title">{title}</h2><Button variant="ghost" size="icon" type="button" onClick={onCancel} disabled={busy} aria-label="Close">×</Button></header><p>{description}</p><footer className="flex justify-end gap-2"><Button ref={cancelRef} type="button" variant="outline" onClick={onCancel} disabled={busy}>Cancel</Button><Button type="button" variant={tone === 'primary' ? 'default' : 'destructive'} onClick={onConfirm} disabled={busy}>{busy ? 'Working…' : confirmLabel}</Button></footer></div></dialog></div>);
}
