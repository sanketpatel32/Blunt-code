...
function OverviewCards({ stats }: { stats?: GlobalStats }) {
  ...
  return <section className="stats-overview" aria-labelledby="stats-overview-title">
    ...
    <div className="stats-grid stats-cards">
      ...
      {data.tools && <article className="summary-card"><strong className="tnum">{data.tools.ready ?? 0}<span className="summary-of"> of {data.tools.total ?? 0}</span></strong><span>Optional tools ready</span><span className="sr-only">Optional analyzer tools reporting ready out of everything installed.</span></article>}
    </div>
