...
const COMMANDS: CLICommand[] = [
  ...
  },
  {
    ...
    synopsis: 'bluntcode tools <list|install|uninstall|repair|update> [args]',
    ...
    subcommands: [
      ...
      { name: 'uninstall', desc: 'Remove downloaded tool files and reclaim disk space', syntax: 'bluntcode tools uninstall <tool-id> [--json]' },
  ...
  {
    id: 'clean',
    name: 'bluntcode clean',
    category: 'Diagnostics & System',
    synopsis: 'bluntcode clean [--logs] [--cache] [--vacuum] [--all] [--json]',
    description: 'Reclaim disk space by purging old scan logs, clearing Trivy vulnerability database caches (~1.3 GB), and defragmenting the SQLite database.',
    badge: 'Maintenance',
    flags: [
      { flag: '--logs', type: 'bool', desc: 'Prune scan logs older than 7 days' },
      { flag: '--cache', type: 'bool', desc: 'Clear Trivy vulnerability database cache' },
      { flag: '--vacuum', type: 'bool', desc: 'Compact SQLite database (VACUUM)' },
      { flag: '--all', type: 'bool', desc: 'Perform all cleanup operations (default if no flags given)' },
      { flag: '--json', type: 'bool', desc: 'Output cleanup summary in JSON format' },
    ],
    examples: [
      { title: 'Clean all caches and prune old logs', cmd: 'bluntcode clean' },
      { title: 'Prune only scan logs older than 7 days', cmd: 'bluntcode clean --logs' },
      { title: 'Clear Trivy vulnerability database cache as JSON', cmd: 'bluntcode clean --cache --json' },
    ],
  },
  {
    id: 'agent',
