...
export function WorkspacesPage({ go, onAdd, notify }: { go: (r: Route) => void; onAdd: () => void; notify: (n: Notice) => void }) {
  ...
  return (
    <div className="page">
      ...
      />
      {state.loading ? <SkeletonCards count={6} variant="workspace" /> : state.error ? <ErrorPanel error={state.error} retry={state.reload} /> : workspaces.length ? <><div className="workspace-controls-row"><div className="workspace-filterbar"><Input type="search" aria-label="Search workspaces" placeholder="Search workspaces…" value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') setSearch(''); }} className="workspace-filter-search w-48 sm:w-60 h-9 text-xs" /><Input type="search" aria-label="Filter by tag" placeholder="Filter by tag…" value={tagQuery} onChange={(event) => setTagQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') setTagQuery(''); }} className="workspace-filter-tag w-36 sm:w-48 h-9 text-xs" />{(debouncedTagQuery||debouncedSearch) ? <button type="button" className="text-button workspace-filter-clear" onClick={()=>{ setTagQuery(''); setSearch(''); }}>Clear</button> : null}
          {(debouncedTagQuery || debouncedSearch) ? <p className="workspace-filter-count" role="status">{visibleWorkspaces.length === workspaces.length ? `${workspaces.length} ${workspaces.length === 1 ? 'workspace' : 'workspaces'}` : `${visibleWorkspaces.length} of ${workspaces.length} ${workspaces.length === 1 ? 'workspace' : 'workspaces'} shown`}</p> : null}</div><WorkspaceSortBar sort={sort} onSort={(key) => setSort((current) => current.key === key ? { key, dir: current.dir === 'asc' ? 'desc' : 'asc' } : { key, dir: firstClickDir[key] })} /></div>{visibleWorkspaces.length ? <div className="workspace-grid">{visibleWorkspaces.map((workspace) => <WorkspaceCard key={workspace.id} workspace={workspace} go={go} notify={notify} onRemoved={state.reload} />)}</div> : debouncedSearch ? <Empty title={`No workspaces match “${debouncedSearch}”`} icon={<FolderIcon />} action={<Button variant="outline" size="sm" onClick={() => setSearch('')}>Clear search</Button>}>No workspace name or path contains “{debouncedSearch}”.</Empty> : <Empty title="No workspaces match this tag" icon={<FolderIcon />}>No workspace tags contain “{tagNeedle}”. Clear the filter to see every project.</Empty>}</> : <><Empty title="No workspaces yet" icon={<FolderIcon />}>Use the folder picker to add a project.</Empty><WorkspaceTemplates onUseTemplate={onAdd} /></>}
    </div>
  );
}
