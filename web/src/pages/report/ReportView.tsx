...
export function ReportView({ scanId, notify, runs: runsProp }: { scanId: string; notify?: (n: Notice) => void; runs?: AnalyzerRun[] }) {
  ...
  // Suppression reasons (IMP-14): a row that says only "suppressed" hides why.
  // The workspace's suppression list supplies the recorded reason per
  // fingerprint; bumping the version refetches it after suppress/restore.
  ...
  /** Recorded suppression reason per finding fingerprint — the "why" behind every suppressed row. */
