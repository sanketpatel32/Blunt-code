...
/** Exports as a flat, always-visible link row — the old popover hid the exact
    thing this page exists to hand over. Kept the component name and props. */
function ExportMenu({ scanId, csvParams, findings }: { scanId: string; csvParams: Record<string, string>; findings?: Finding[] }) {
  ...
  return <fieldset className="export-menu export-inline" aria-label="Export report"><span className="export-label">Export</span>{items.map((item) => <a key={item.label} className="export-item" href={item.href} download>{item.label}<small>{item.ext}</small></a>)}<button type="button" className="export-item" onClick={handleJiraCsv}>Jira CSV<small>.csv</small></button></fieldset>;
}
...
function FindingFilters({ filters, setFilters, analyzers, rules, onAnalyzer }: { filters: FindingFilter; setFilters: (next: FindingFilter) => void; analyzers?: AnalyzerRun[]; rules: string[]; onAnalyzer: (analyzer: string) => void }) { const set = (key: keyof FindingFilter, value: string) => setFilters({ ...filters, [key]: value }); const tools = [...new Set(analyzers?.map((run) => run.analyzer_id) ?? [])]; return <div className="filters" id="finding-filters"><div className="filters-field"><span className="filters-field-label">Tool</span><fieldset className="chip-group" aria-label="Tool"><button type="button" className="chip" aria-pressed={filters.analyzer === ''} onClick={() => onAnalyzer('')}>All tools</button>{tools.map((tool) => <button type="button" key={tool} className="chip" aria-pressed={filters.analyzer === tool} onClick={() => onAnalyzer(tool)}>{analyzerName(tool)}</button>)}</fieldset></div><label>Category<input value={filters.category} onChange={(e) => set('category', e.target.value)} placeholder="All categories" /></label><label>Rule<select value={filters.rule} onChange={(e) => set('rule', e.target.value)}><option value="">All rules</option>{rules.map((rule) => <option value={rule} key={rule}>{rule}</option>)}</select></label><label>File<input value={filters.path} onChange={(e) => set('path', e.target.value)} placeholder="Any path" /></label><div className="filters-field"><span className="filters-field-label">Status</span><fieldset className="chip-group" aria-label="Status"><button type="button" className="chip" aria-pressed={filters.status === ''} onClick={() => set('status', '')}>All</button><button type="button" className="chip" aria-pressed={filters.status === 'new'} onClick={() => set('status', 'new')}>New</button><button type="button" className="chip" aria-pressed={filters.status === 'persistent'} onClick={() => set('status', 'persistent')}>Persistent</button><button type="button" className="chip" aria-pressed={filters.status === 'suppressed'} onClick={() => set('status', 'suppressed')}>Suppressed</button></fieldset></div><label className="filter-search">Search<input value={filters.q} onChange={(e) => set('q', e.target.value)} placeholder="Message or rule" /></label></div>; }
...
function FindingsPagination({ total, page, pageSize, hasNext, onPrevious, onNext, onPageSize }: { total: number; page: number; pageSize: number; hasNext: boolean; onPrevious: () => void; onNext: () => void; onPageSize: (size: number) => void }) { const pageCount = Math.max(1, Math.ceil(total / pageSize)); const first = total ? (page - 1) * pageSize + 1 : 0; const last = Math.min(page * pageSize, total); return <nav className="findings-pagination" aria-label="Findings pagination"><span>Showing {first}–{last} of {total}</span><div><span className="page-size-label">Per page</span><fieldset className="segmented page-size" aria-label="Rows per page">{PAGE_SIZE_OPTIONS.map((size) => <button key={size} type="button" aria-pressed={pageSize === size} onClick={() => { if (size !== pageSize) onPageSize(size); }}>{size}</button>)}</fieldset><button type="button" className="button secondary" onClick={onPrevious} disabled={page <= 1}>Previous</button><output aria-live="polite">Page {page} of {pageCount}</output><button type="button" className="button secondary" onClick={onNext} disabled={!hasNext}>Next</button></div></nav>; }
