...
export function HistoryPage({ workspaceId, go }: { workspaceId: string; go: (r: Route) => void }) {
  ...
  // The paging envelope's total degrades to the loaded item count so a missing
  // or partial envelope never renders "undefined scans" in the badge or count.
  const totalScans = state.data?.total ?? state.data?.items?.length ?? 0;
  useEffect(() => {
...
export function HistoryTable({ scans, go, paging, dateFrom, dateTo }: { scans: Scan[]; go: (r: Route) => void; paging?: HistoryPaging; dateFrom?: string; dateTo?: string }) {
  ...
  const toggleExpanded = (id: string) => setExpanded((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; });  const serverMode = !!paging;
  ...
  const dateFiltered = scans.filter((s) => {
    ...
    // Parse the date inputs as LOCAL day bounds (`new Date('2026-09-06')` alone
    // reads as UTC midnight, so UTC+ users lost the 00:00-05:30 window).
