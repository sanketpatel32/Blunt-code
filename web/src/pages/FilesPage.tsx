...
export function FilesPage({ id, go, notify }: { id: string; go?: (r: Route) => void; notify: (n: Notice) => void }) {
  ...
  return <div className="page workspace-page">{go && <WorkspaceContextSidebar id={id} current={{ page: 'files', id }} onNavigate={go} />}<div className="workspace-page-body"><header className="page-heading workspace-heading"><div className="workspace-hero-copy"><p className="eyebrow">File selection</p><h1>{workspace.data?.name ?? 'Workspace files'}</h1><code className="workspace-root" title={workspace.data?.root_path}>{workspace.data?.root_path}</code><p className="muted">Choose source paths to analyze. Default exclusions protect dependencies and build output.</p></div><div className="files-toolbar"><button type="button" className="button ghost files-reset" onClick={() => { setRules({ rules: [] }); setOverrides([]); }}><RotateCcw size={14} aria-hidden />Reset</button><button type="button" className="button primary files-save" onClick={save}><Save size={14} aria-hidden />Save selection</button></div></header>
    <section className="file-layout"><div className="tree-panel"><div className="tree-panel-head"><div><p className="eyebrow">Source tree</p><h2 className="tree-panel-title">Workspace files</h2></div><span className="tree-count-badge tabular-nums">{nodes.length ? `${nodes.length} top-level` : '—'}</span></div><div className="tree-panel-controls"><label className="search file-search"><span className="sr-only">Search paths</span><span className="file-search-wrap"><Search size={14} className="file-search-icon" aria-hidden /><input ref={searchRef} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') { setQuery(''); event.currentTarget.blur(); } }} placeholder="src or package.json" className="file-search-input" /><kbd className="kbd-hint">/</kbd></span></label><div className="tree-toolbar"><select aria-label="Filter by language" value={lang} onChange={e=>setLang(e.target.value)} className="file-lang-select">
        ...
        </select>{lang && <button type="button" className="text-button" onClick={() => setLang('')}>Clear filter</button>}{!loadingTree && !treeError && <button type="button" className="button ghost tree-collapse-btn" onClick={() => setCollapseSignal((value) => value + 1)}>Collapse all</button>}</div></div><div className="tree-scroll">{loadingTree ? <SkeletonLines lines={6} /> : treeError ? <ErrorPanel error={treeError} retry={loadTree} /> : <FileTree key={treeKey} nodes={nodes} query={debouncedQuery} lang={debouncedLang} workspaceId={id} overrides={overrides} onOverrides={setOverrides} collapseSignal={collapseSignal} />}</div><div className="tree-summary-bar"><span className="tabular-nums">{selectedCount ? `${selectedCount} paths selected` : `${nodes.length} paths`}</span><span aria-hidden>·</span><span className="tabular-nums">{langDistinct} languages</span></div></div><RuleEditor rules={rules.rules} setRules={(items) => setRules({ rules: items })} /></section>
  </div></div>;
}
