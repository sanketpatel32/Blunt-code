...
import { FileCode, FileJson, FileText, FileBraces, Braces, Palette, Database, Container, Settings2, ScrollText, Code2, FileSpreadsheet, Search, Plus, X } from 'lucide-react';
import { api } from '../api';
...
function RuleEditor({ rules, setRules }: { rules: RuleDraft[]; setRules: (next: RuleDraft[]) => void }) {
  ...
  return <aside className="rule-editor"><header className="rule-editor-head"><div><p className="eyebrow">Overrides</p><h2>Rules</h2></div></header><p className="muted">Rules apply to new files too.</p>{rules.length === 0 && <p className="rule-empty muted">No custom rules yet. Add one to include or exclude paths beyond defaults.</p>}{rules.map((rule) => <div className="rule" key={rule.uid}><select value={rule.rule_type} onChange={(event) => edit(rule.uid, { rule_type: event.target.value as 'include' | 'exclude' })} aria-label="Rule type"><option value="include">Include</option><option value="exclude">Exclude</option></select><input value={rule.pattern} onChange={(event) => edit(rule.uid, { pattern: event.target.value })} placeholder="e.g. **/generated/**" aria-label="Rule pattern" /><button type="button" className="icon-button rule-remove" onClick={() => setRules(rules.filter((item) => item.uid !== rule.uid))} aria-label="Remove rule"><X size={14} /></button></div>)}<button type="button" className="rule-add" onClick={() => setRules([...rules, { uid: nextRuleUid(), rule_type: 'exclude', pattern: '' }])}><Plus size={14} aria-hidden /> Add rule</button></aside>;
}
