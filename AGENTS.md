...
- Build the release zip from the exact tagged commit, not the shared working tree (a paired session's uncommitted edits must never ride a release): `git worktree add <tmp> <commit>` → `go build -ldflags="-s -w" -o bluntcode.exe ./cmd/bluntcode` → `scripts/package.ps1 -SkipBuild -Version X.Y.Z -OutputDir <repo>\dist` → `git worktree remove <tmp>`
