...
### Added
- **Tool Uninstaller & Disk Usage Visibility**: `bluntcode tools uninstall <tool>` CLI command, `DELETE /api/v1/tools/{id}` endpoint, and web UI Tools page uninstall action with confirmation dialog. Uninstalls optional analyzers and deletes their runtime/data directory (e.g. SonarQube data + binary), reclaiming gigabytes of disk space. Disk usage is tracked and reported per tool.
- **Storage Cleaner**: `bluntcode clean [--logs] [--cache] [--database] [--all]` command and `POST /api/v1/system/clean` API endpoint. Prunes logs older than 7 days, empties Trivy vulnerability cache, and runs SQLite `VACUUM` on `bluntcode.db`. Exposed in web Settings page with itemized breakdown and one-click cleanup.
- **Automatic Scan History Retention**: Automatically limits scans per workspace (capped at 20 scans by default) after scan completion, pruning aged findings and artifacts to prevent unbounded database growth.
- **Packaging Binary Budget Guard**: Built-in <= 18 MB binary budget check in `scripts/package.ps1` to prevent bloated release binaries.

### Changed
- **Binary Size Stripping**: Stripped Go binaries with `-ldflags="-s -w"` in release builds, reducing the `bluntcode.exe` executable footprint from 23.5 MB to 15.9 MB (~32% reduction).
- **Frontend & CSS Diet**: Cleaned unused CSS across `styles.css`, `nav-clarity.css`, and `animations.css`; removed unused Vite manual chunk configurations; disabled eager preloading of non-critical route chunks.
...
### Fixed
- **Repository & Workspace Hygiene**: Hardened `.gitignore` to prevent editor backups (`*~`, `*.bak`), dumps (`*.dmp`), Playwright artifacts, vitest cache, and ruff cache from polluting git tracking.
...
- **Versioned finding identity with coverage-aware baselines (IMP-08)**: fingerprints carry a schema version, separate logical-issue from occurrence identity (repeated identical lines no longer collapse), and prior-scan comparisons mark findings `not_evaluated` instead of `Fixed` when the analyzer that would have caught them was disabled, failed, or had its inputs excluded; SARIF and prior-scan baselines classify identically where their evidence is equivalent, and suppressions never silently broaden when fingerprint logic changes.
- **Correctness-first incremental reuse (IMP-09)**: cache eligibility keys on input digests, analyzer/rule/parser versions, and effective configuration rather than mtime alone; deletions, renames, ignore/profile changes, dependency changes, and tool/rule/data updates invalidate reuse; interrupted or unparseable output is never reused as a complete result, and scan metadata tracks reused vs freshly evaluated findings with gate semantics identical to a full scan.
- **Crash-safe update and install pipeline (IMP-10)**: managed-tool archives extract to a staged directory, reject zip-slip paths before any write, verify content hashes, and swap atomically with rollback on failure.
- **Crash recovery and retention transparency (IMP-11)**: scans interrupted by an application crash are relabeled `interrupted` at startup with their saved partial results visible; database ownership is coordinated so a stale process cannot interleave writes; pruning reports exactly which scans it deleted (the API returns the deleted ids) and refuses to prune scans still referenced as baselines.
- **Hardened export guarantees (IMP-12)**: every exported artifact (HTML, CSV, GitHub annotations, JSON, JSONL, SARIF, markdown, saved baselines) is written atomically — temp file, sync, rename — so a crash can never leave a truncated report; SARIF `helpUri` values are restricted to http(s); CSV neutralizes formula injection (`=`, `+`, `-`, `@`, tab/CR prefixes); GitHub workflow-command output escapes `%`, newlines, and `::`; control characters are scrubbed. A hostile-corpus acceptance test pushes one malicious fixture through every exporter.
- **Coverage-paired risk and evidence-grade metrics (IMP-13)**: the risk endpoint and workspace cards pair every grade with the scan's state, finish time, analyzer coverage, and a `complete` flag — a partial scan can no longer present an unqualified grade; severity mappings from raw analyzer values are versioned (`SeverityMappingVersion`), with unmapped values marked (`severity_unmapped` metadata, raw severity preserved) instead of silently defaulted; `docs/metrics.md` documents the counting semantics (active vs suppressed, new/persistent/fixed/not_evaluated, duplicates vs corroboration, risk ledger weights and bands).
- **Understandable progress and coverage in the UI (IMP-14)**: the scan page shows a literal state badge and the failure reason for scans that never got healthy (no eligible inputs, offline analyzers); the cancel button guards against double-fire; starting a scan while another is active surfaces the API's `SCAN_ALREADY_ACTIVE` with a "View scan" action instead of a generic error; a pre-scan summary card on the workspace page predicts which engines will run before you commit; history rows expose what discovery selected and skipped (with human reasons); suppressed findings show their suppression reason inline.
- **Release evidence map (IMP-15)**: `docs/release-evidence.md` maps every P0 acceptance case to the automated test that verifies it or a reproducible manual procedure, inventories the fake-analyzer scenario matrix and golden parser fixtures, records the packaging smoke procedure against release zips, and lists the honestly-open items (benchmarks, performance budgets, environmental matrix).
- **Accessibility evidence and narrowed conformance claim (IMP-16)**: `docs/accessibility.md` states exactly what is verified — automated contrast auditing, jsdom-tested keyboard/dialog/focus/live-region behavior, a keyboard journey test through the core flow (workspace → scan → incomplete result → finding → back out, no focus trap), router edge tests (popstate back/forward, unknown ids, hostile URL parameters), and i18n fallback tests — and scopes the WCAG statement to that evidence instead of claiming full 2.2 AA conformance.

### Changed
- **CLI manual and usage now contract-tested against the parser (IMP-16)**: every flag the scan command accepts must appear in both the usage line and the interactive manual — this found and fixed eight undocumented flags (`--save-baseline`, `--gate-analyzer`, `--gate-category`, `--watch-poll`, `--watch-quiet`, `--github-cap`, `--json`, `--timeout`) and the missing `pentest` profile in the manual.
- **Dates and scan states stay honest across locales**: no translation key may shadow internal severity or scan-state identifiers (pinned by test); switching UI language updates `<html lang>` and persists.

### Fixed
- **Command palette "Go to findings" landed on the 404 page (IMP-16)**: `href()` builds `/search` for the search route but the router never parsed `/search` back — only the `/findings` alias worked. `/search` is now canonical.
- **Stale agent docs in the web app**: `web/public/llms.txt` / `llm.txt` still advertised 0.16.21; they now match the shipped 0.22.0.
...
### Added
- **Smart skip for generated files**: discovery no longer reads files nobody hand-edits. New defaults exclude build and dependency directories across ecosystems (`target`, `vendor`, `obj`, `Pods`, `.svelte-kit`, `.output`, `.turbo`, `site-packages`, `.terraform`, `playwright-report`, `log`/`logs`, and ~30 more), minified and bundled output wherever it sits (`*.min.css`, `*.chunk.js`, `*.bundle.js`, `vendor.js`, numeric webpack chunks, and hash-suffixed bundler output like `index-CKc0XBc7.js` — Vite/esbuild/webpack shapes, matched without touching hand-written names like `service2.js`), dependency lockfiles (`package-lock.json`, `pnpm-lock.yaml`, `yarn.lock`, …) whose multi-megabyte JSON only produced analyzer grind and sha512 false positives, generated code (`*.pb.go`, `*_pb2.py`, `*.generated.ts`, `*.designer.cs`), `*.tsbuildinfo`, files over 10 MiB, and any large text file whose head contains a 500+-character line — the signature of a minified bundle hiding under a source-looking name (found while dogfooding: scans of this very repository were reading their own compiled web bundle, `cmd/bluntcode/static/assets/index-*.js`).
- **Findings inside generated output are dropped at the pipeline**: directory-walking analyzers (sonarqube, semgrep-style walkers, trivy, checkov) traverse build output no matter what discovery selected, so code-smell findings on `dist/` bundles flooded the findings table. Findings on artifact paths are now filtered before persistence. Two deliberate exceptions: secret detectors (secrets, gitleaks) keep artifact findings — a credential baked into a shipped bundle or vendored dependency is exactly the leak a scan must surface — and lockfile paths are never treated as artifacts, so osv/trivy dependency findings on `package-lock.json` and friends survive.
...
### Changed
- **Notifications bell joined the nav's utility cluster**: it was a bordered box outside the group with two overlapping red indicators (a corner badge plus a decorative dot) that read as a smear — it now styles like its ghost siblings and carries a single, legible unread-count badge.
...
### Changed
- **Dashboard redesigned from scratch as a Risk Board**: the dashboard now answers three questions in reading order — *how risky is my code right now?* (grade tile, A/B/C/D band ruler, stacked severity tally, live stats rail), *where is the risk?* (a ranked ledger of workspaces, worst first, each with its own grade, severity bar, and weighted score), and *what happened lately?* (real severity trend plus a scannable feed with state filters). The grade language now matches the backend risk profile exactly (critical ×10, high ×5, medium ×2, low ×1; bands A <5, B <20, C <50, D 50+) via a shared `web/src/lib/risk.ts`, replacing the dashboard's old invented formula, and every number on the page comes from real scan data — the fake seed-based sparklines and synthetic language charts are gone (only ever-rendered bars from actual scan totals remain). Ledger rows are compact two-line rows with language chips capped at four plus an overflow count, onboarding for the first workspace is unchanged, and the engines strip reports real tool readiness.
